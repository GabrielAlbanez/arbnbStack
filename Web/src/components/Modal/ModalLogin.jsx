...
export default function ModalLogin({ handleClose }) {
    ...
    const [forgetPasswordClick, setForgetPasswordClick] = useState(false)
    ...
    const handleSubmitForgerPassword = async(e)=>{
        e.preventDefault()
        try{

            const response = await fetch("http://localhost:8080/fogetPassword",{
                method : "POST",
                headers: {
                    "Content-Type": "application/json",
                },
            ...
            if ("error" in responseData) {
                ...
                setshowToas(true)
                setToasterMEssage(responseData.message)
                setForgetPasswordClick(false)
            }
            ...
            {!forgetPasswordClick ?
                        ...
                        <main className=' h-[88%]  '>
                            ...
                            <form onSubmit={handleSubmit} className='flex flex-col justify-evenly  py-14 items-center w-[100%] h-[68%]'>
                                ...
                                <div className='flex justify-between px-4 text-[14px] w-[70%] '>
                                    <div>Lembrar</div>
                                    <div onClick={() => { setForgetPasswordClick(true) }} className='cursor-pointer'>Esqueci minha Senha</div>
                                </div>
                ...
                : (<>
                    ...
                    <div className='bg-white w-[60vh] h-[70vh] rounded-md'>
                        ...
                        <main className=' h-[88%]  '>
                            <div className='text-2xl flex items-end justify-center pt-7  h-[20%]'>Esqueci Senha</div>
                            <form onSubmit={handleSubmitForgerPassword} className='flex flex-col justify-evenly  py-14 items-center w-[100%] h-[68%]'>
                                ...
                                <div className='flex flex-col gap-2'>
                                    <div><label htmlFor="ip1">Senha Atual:</label></div>
                                    <div className='w-[40vh] border-rose-500 border-[1px] flex items-center justify-center h-[50%] rounded-full transition shadow-rose-200 shadow-md hover:shadow-lg hover:shadow-rose-400 '><input name='senha' value={dataLogin.senha} onChange={hanleInputChange} type="password" placeholder='insira sua senha..' className='w-[93%] rounded-full h-[60%] border-white outline-0 bg-transparent p-3' /></div>
                                ...
                                <div className='flex flex-col gap-2 pt-10 '>
                                    <button type='submit' className='border-rose-500 rounded-full border-[1px] h-[4vh] w-[20vh] transition hover:text-white hover:bg-rose-500'>Salvar</button>
                                </div>
                            </form>
                        </main>
